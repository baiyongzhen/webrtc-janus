...
func coreInputState(port C.unsigned, device C.unsigned, index C.unsigned, id C.unsigned) C.int16_t {
	if device == C.RETRO_DEVICE_ANALOG {
		...
		axis := index * 2 + id
		log.Println("1 port:", port)
		log.Println("1 device:", device)
		log.Println("1 index:", index)
		log.Println("1 id:", id)
		log.Println("1 id:", axis)
	...
	if id >= 255 || index > 0 || device != C.RETRO_DEVICE_JOYPAD {
		log.Println("2 port:", port)
		log.Println("2 device:", device)
		log.Println("2 index:", index)
		log.Println("2 id:", id)
		return 0
	}
	...
	if !ok {
		log.Println("3 port:", port)
		log.Println("3 device:", device)
		log.Println("3 index:", index)
		log.Println("3 id:", id)
		return 0
	}
	...
	for k := range NAEmulator.controllersMap {
		if ((NAEmulator.controllersMap[k][port].keyState >> uint(key)) & 1) == 1 {
			log.Println("4 port:", port)
			log.Println("4 device:", device)
			log.Println("4 index:", index)
			log.Println("4 id:", id)
			log.Println("4 key:", key)
			return 1
		}
	}
	return 0
