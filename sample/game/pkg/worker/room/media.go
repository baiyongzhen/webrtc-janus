...
import (
	...
	"example.com/webrtc-game/pkg/encoder"
	"example.com/webrtc-game/pkg/encoder/h264encoder"
	vpxencoder "example.com/webrtc-game/pkg/encoder/vpx-encoder"
...
func (r *Room) startVoice() {
	log.Println("Enter fan startVoice")
	// broadcast voice
	go func() {
	...
	go func() {
		for sample := range r.voiceOutChannel {
			//log.Println("startVoice:voiceOutChannel")
			for _, webRTC := range r.rtcSessions {
				if webRTC.IsConnected() {
					// NOTE: can block here
					log.Println("startVoice")
...
func (r *Room) startAudio(sampleRate int) {
	...
	for sample := range r.audioChannel {
		//log.Println("startAudio:audioChannel")
...
func (r *Room) startVideo(width, height int, videoEncoderType string) {
	...
	go func() {
		...
		for data := range eoutput {
			//log.Println("startVideo:eoutput")
			// TODO: r.rtcSessions is rarely updated. Lock will hold down perf
			for _, webRTC := range r.rtcSessions {
				// encode frame
				// fanout imageChannel
	...
	for image := range r.imageChannel {
		//log.Println("startVideo:imageChannel")
